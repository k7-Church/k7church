...

{
    ...
    public function ch_register()
    {
        ...
        add_shortcode('nejo' , array($this , 'ch_event_shortcode_output'));
    ...
    {
        add_meta_box('ept_event_date_start' , 'Start Date and Time' , array($this , 'ept_event_date') , 'event' , 'side' , 'default' , array('id' => '_start'));
        add_meta_box('ept_event_date_end' , 'End Date and Time' , array($this , 'ept_event_date') , 'event' , 'side' , 'default' , array('id' => '_end'));
        add_meta_box('ept_event_location' , 'Event Location' , array($this , 'ept_event_location') , 'event' , 'normal' , 'default' , array('id' => '_end'));
    }

    // Metabox HTML
    public function ept_event_date($post , $args)
    {
    ...
    public function ept_event_location()
    {
        global $post;
        // Use nonce for verification
        wp_nonce_field(plugin_basename(__FILE__) , 'ch_eventposts_nonce');
            ...
            <li>
                <ul>
                    ...
                    if ($end < $current_timestamp) {
                        $started = "\t\n<b style='border: 6px solid red;'>\t\n" . esc_html__('Has finished' , 'k7') . '</b>';

                    }
                    if ($end > $current_timestamp && $current_timestamp < get_post_meta($event->ID , '_start_eventtimestamp' , true)) {
                        $started = "\t\n<b style='border: 6px solid green;'>\t\n" . esc_html__('It will start soon' , 'k7') . '</b>';

                    } ?>
                    <li><?php _e('Event start date:' , 'k7');
                        echo "\t\n".$month . ' ' . $day . ' ' . $year; ?></li>
                    <li><?php _e('Start event timestamp:' , 'k7');
                        echo "\t\n".get_post_meta($event->ID , '_start_eventtimestamp' , true) . $started; ?></li>
                    <li><?php _e('End event timestamp:' , 'k7');
                        echo "\t\n".$end; ?></li>
                    <li><?php _e('Current timestamp:' , 'k7');
...
<?php
            ...
            foreach ($events as $event) {
                ...
                <ul>
                    ...
                    if ($end > $current_timestamp && $current_timestamp < get_post_meta($event->ID , '_start_eventtimestamp' , true)) {
                        $started = "\t\n<b style='border: 6px solid ". get_option('event_status_soon').";'>\t\n" . esc_html__('It will start soon' , 'k7') . '</b>';
